...
We study binary intrusion detection on the NSL-KDD benchmark and compare a classical baseline like logistic regression with a multilayer perceptron known as MLP. Because missed attacks are costly and
...
challenge is data imbalance, meaning benign traffic may dominate, and certain attack patterns can be under-represented.
...
Our repository includes a README with step-by-step commands to reproduce preprocessing, training, and table/figure generation.
...
NSL-KDD still remains a common reference point for controlled evaluations, despite not being a modern enterprise traffic dataset.
...
Autoencoder ensembles have also been proposed for online anomaly detection, emphasizing adaptive representations and
...
We address network intrusion detection as a supervised binary classification problem on the NSL-KDD benchmark.
Each network connection is labeled as either normal (0) or attack (1). The goal is to learn a model that
detects attacks reliably while keeping false alarms manageable.

As false negatives are particularly costly in security settings and IDS datasets are often class-imbalanced,
we evaluate models primarily using attack-class Recall and F1-score, with accuracy and precision reported as supporting metrics.
We compare a classical baseline against a simple deep learning approach and include an
imbalance-aware training extension to test whether it improves detection of attacks.

\section{4. Approach}

\subsection{4.1 Baseline: Logistic Regression}
...
\subsection{4.2 Deep Model: Multilayer Perceptron}
...
\subsection{4.3 Imbalance Handling Extension}
...
\subsection{4.4 Hyperparameter Selection and Training Details}
...
\section{5 Data and Experimental Setup}
We use NSL-KDD, where each instance corresponds to a network connection described by mixed feature types, including
categorical attributes (for example, protocol type, service, flag) and numeric attributes (byte counts, error rates, traffic statistics, etc.). We convert original labels into a binary target: normal (0) versus attack (1). The official
dataset provides separate train and test files; we further split the training file to create a validation set.

\subsection{5.1 Preprocessing Pipeline}
...
Validation performance is substantially higher than test performance. This behavior is consistent with known differences
